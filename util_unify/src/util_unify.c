...
#include "downlink.h"   // Functions for packets to be sent to a gateway
#include "uplink.h"     // Functions for packets to be received from a gateway
...
int main(int argc, char **argv)
{
    ...
    if (argc != 2) {
        MSG("Usage: util_unify <port number>\n");
        exit(EXIT_FAILURE);
    }
